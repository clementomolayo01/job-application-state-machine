...
describe('ApplicationsService', () => {
    ...
    it('should allow INTERVIEWING -> CONTRACTED with contractUrl', () => {
      expect(() => {
        service.validateTransition(
          ApplicationStatus.INTERVIEWING,
          ApplicationStatus.CONTRACTED,
          { status: ApplicationStatus.CONTRACTED, contractUrl: 'http://example.com' }
        );
      }).not.toThrow();
    });
  ...
  describe('updateStatus', () => {
    it('should throw NotFoundException if app does not exist', async () => {
      ...
      await expect(
        service.updateStatus('uuid', { status: ApplicationStatus.INTERVIEWING }, 'user1')
      ).rejects.toThrow(NotFoundException);
    });

    it('should perform atomic transaction and send email on CONTRACTED', async () => {
      const mockApp = { id: 'uuid', status: ApplicationStatus.INTERVIEWING, email: 'test@test.com' } as any;
      ...
      const emailSpy = jest.spyOn(emailService, 'sendContractConfirmation').mockResolvedValue(undefined);
  ...
  describe('getHistory', () => {
    it('should return history', async () => {
      ...
      jest.spyOn(prisma.statusHistory, 'findMany').mockResolvedValue(historyResult);
